...
const Navbar = () => {
  ...
  return (
    <header className="flex sticky top-0 z-50 justify-between ml-2 rounded-[6px]  mt-2 px-8 secondary_bg items-center ">
      <div className="flex gap-2 flex flex-col md:flex-row items-center  w-1/2">
        ...
        <div
          ...
          <input
            ...
            className={`block  w-full rounded-full pl-9 border-0  text-gray-300 shadow-sm ring ring-transparent placeholder:text-gray-400 focus:ring-[3px] focus:ring-inset focus:ring-white outline-none p-3 hover:ring-white/20 bg-[#1a1919] `}
          />
      ...
      <div>
        {!isAuthenticated ? (
          <div className="flex">
            <Link
              to={"/signup"}
              className="rounded-full  mt-4 px-8 text-base  py-2 text-white- font-semibold hidden lg:block">
              Sign Up
            </Link>

            <Link
              to={"/login"}
              className="rounded-full text-black mt-4 px-4 lg:px-8 text-base  py-3 bg-white font-semibold text-xs lg:text-base">
              Log in
            </Link>
          </div>
        ) : (
